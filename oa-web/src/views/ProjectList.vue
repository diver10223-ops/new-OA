...
async function load() {
  loading.value = true
  try {
    const r: any = await projectApi.list(q)
...
async function edit(row: any) {
  const r: any = await projectApi.detail(row.id)
...
async function save(submit = false) {
  ...
  try {
    ...
    if (id) {
      ...
      const r: any = await projectApi.create(form)
      id = r.data.id
    }
    if (submit && id) {
...
async function action(row: any, type: 'remove' | 'submit' | 'withdraw' | 'close') {
...
<template>
    ...
    <el-form inline>
      <el-form-item label="状态">
        <el-select v-model="q.status" clearable style="width: 140px">
          <el-option
            v-for="s in ['DRAFT', 'SUBMITTED', 'ESTABLISHED', 'REJECTED', 'CANCELLED', 'CLOSED']"
            :key="s"
            :label="statusLabel(s)"
            :value="s"
          />
      ...
      <el-form-item label="关键字">
        <el-input v-model="q.keyword" clearable placeholder="项目名称或编码" />
      </el-form-item>

      <el-form-item>
        <el-button type="primary" @click="q.page = 0; load()">查询</el-button>
        <el-button type="success" @click="create">新建立项</el-button>
      </el-form-item>
    </el-form>
  </el-card>

  <el-card>
    <el-table :data="rows" v-loading="loading" empty-text="暂无立项申请">
      <el-table-column prop="applicationNo" label="申请单号" />
      <el-table-column prop="projectName" label="项目名称" />
      <el-table-column prop="projectOwner" label="项目负责人" />
      <el-table-column prop="budget" label="预算" />
      <el-table-column label="状态">
        <template #default="scope">
          <el-tag :type="statusType(scope.row.businessStatus)">
            {{ statusLabel(scope.row.businessStatus) }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="310">
        <template #default="scope">
          <el-button link @click="router.push('/project/' + scope.row.id)">详情</el-button>
          <el-button
            v-if="scope.row.businessStatus === 'DRAFT'"
            link
          ...
          <el-button
            ...
            link
            type="danger"
            @click="action(scope.row, 'remove')"
          >
            删除
          </el-button>
          <el-button
            v-if="scope.row.businessStatus === 'SUBMITTED'"
            link
            type="warning"
            @click="action(scope.row, 'withdraw')"
          >
            撤回
          </el-button>
          <el-button
            v-if="scope.row.businessStatus === 'ESTABLISHED'"
            link
            @click="action(scope.row, 'close')"
          >
            结项
          </el-button>
        </template>
  ...
  <el-dialog v-model="dialog" :title="editingId ? '编辑立项草稿' : '新建立项申请'" width="620px">
      ...
      <el-row :gutter="16">
        <el-col :span="12">
          <el-form-item label="开始日期" prop="plannedStartDate">
            <el-date-picker v-model="form.plannedStartDate" value-format="YYYY-MM-DD" />
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item label="结束日期" prop="plannedEndDate">
            <el-date-picker v-model="form.plannedEndDate" value-format="YYYY-MM-DD" />
          </el-form-item>
        </el-col>
      </el-row>
