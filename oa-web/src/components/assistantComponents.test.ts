...
import type { AssistantResult, ExecutionTrace } from '../types/assistant'
...
function mountIndicator(data: AssistantResult) {
...
function mountTrace(props: { trace: ExecutionTrace | null; loading?: boolean; error?: string }) {
...
describe('TraceDrawer', () => {
  it('renders all seven stages in original order with time and details', () => {
    ...
    labels.forEach((label, index) => {
      ...
      if (index > 0) expect(content.indexOf(labels[index - 1])).toBeLessThan(content.indexOf(label))
    })
