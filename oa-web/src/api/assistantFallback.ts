import type { AssistantExecuteRequest, AssistantExecuteResponse, ExecutionTrace, ExecutionTraceEvent } from '../types/assistant'

const traces = new Map<string, ExecutionTrace>()
const suggestions = ['本分行存款余额是多少', '总部平均存款比上期增加多少', '贷款占比近三个月趋势']

function makeResponse(request: AssistantExecuteRequest): AssistantExecuteResponse {
  ...
  let response: AssistantExecuteResponse
