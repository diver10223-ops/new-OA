...
class CoreBusinessFlowTest {
    ...
    void leaveSupportsDraftEditSubmitApproveAndRejectWithPermissionBoundaries() throws Exception {
        ...
        assertThat(status("leave_application", approved)).isEqualTo("EFFECTIVE");
        ...
        assertThat(status("leave_application", rejected)).isEqualTo("REJECTED");
    }
    ...
    void withdrawalClosesPendingTaskAndSynchronizesLeaveState() throws Exception {
        ...
        mvc.perform(post("/api/leave-applications/{id}/withdraw", id).header("Authorization", employee))
                .andExpect(status().isOk());
        assertThat(status("leave_application", id)).isEqualTo("CANCELLED");
    ...
    void projectApprovalIsStrictlySerialAndFinalCallbackEstablishesProject() throws Exception {
        ...
        assertThat(status("project_application", id)).isEqualTo("SUBMITTED");
        ...
        assertThat(status("project_application", id)).isEqualTo("ESTABLISHED");
    }
    ...
    void firstProjectRejectionDoesNotCreateReviewTask() throws Exception {
        ...
        assertThat(status("project_application", id)).isEqualTo("REJECTED");
        assertThat(taskCount(id, "PROJECT_REVIEW")).isZero();
    }
    ...
    void projectCanBeWithdrawnBeforeFirstDecision() throws Exception {
        ...
        mvc.perform(post("/api/project-applications/{id}/withdraw", id).header("Authorization", employee))
                .andExpect(status().isOk());
        assertThat(status("project_application", id)).isEqualTo("CANCELLED");
        assertThat(db.queryForObject("select status from approval_task where id=?", String.class, task)).isEqualTo("CANCELLED");
        decide(manager, task, true).andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value(40902));
    }
    ...
    void optimisticLockConflictRollsBackTaskDecision() throws Exception {
        ...
        assertThat(status("leave_application", id)).isEqualTo("EFFECTIVE");
    }
    ...
    private String leaveJson(String reason) { return "{\"leaveType\":\"ANNUAL\",\"startTime\":\"2030-01-01T09:00:00Z\",\"endTime\":\"2030-01-01T18:00:00Z\",\"reason\":\"" + reason + "\"}"; }
    private void submitLeave(String auth, long id) throws Exception { mvc.perform(post("/api/leave-applications/{id}/submit", id).header("Authorization", auth)).andExpect(status().isOk()); }
    ...
    private void submitProject(String auth, long id) throws Exception { mvc.perform(post("/api/project-applications/{id}/submit", id).header("Authorization", auth)).andExpect(status().isOk()); }
    ...
    private int decisionStatus(String auth, long task) { try { return decide(auth, task, true).andReturn().getResponse().getStatus(); } catch (Exception ex) { throw new RuntimeException(ex); } }
    private long pendingTask(String type, long businessId) { return db.queryForObject("select t.id from approval_task t join approval_instance i on i.id=t.instance_id where i.business_type=? and i.business_id=? and t.status='PENDING'", Long.class, type, businessId); }
    private String status(String table, long id) { return db.queryForObject("select business_status from " + table + " where id=?", String.class, id); }
    private int taskCount(long businessId, String node) { return db.queryForObject("select count(*) from approval_task t join approval_instance i on i.id=t.instance_id where i.business_type='PROJECT' and i.business_id=? and t.node_code=?", Integer.class, businessId, node); }
}
