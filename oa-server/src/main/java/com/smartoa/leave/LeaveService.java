...
public class LeaveService {
    ...
    private void validatePage(int page,int size){if(page<0||size<1||size>100)throw new BusinessException(40002,"分页参数范围为 page>=0 且 1<=size<=100");}
    private Map<String,Object> oneForDetail(long id){return db.queryForMap("select l.id,l.application_no applicationNo,l.applicant_id applicantId,u.display_name applicantName,l.department_id departmentId,d.name departmentName,l.leave_type leaveType,l.start_time startTime,l.end_time endTime,l.duration,l.reason,l.business_status businessStatus,l.approval_instance_id approvalInstanceId,l.created_at createdAt,l.updated_at updatedAt from leave_application l join sys_user u on u.id=l.applicant_id join sys_department d on d.id=l.department_id where l.id=?",id);}
