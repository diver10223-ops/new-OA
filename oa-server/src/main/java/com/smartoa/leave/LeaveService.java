...
public class LeaveService {
    ...
    @Transactional public void withdraw(long id){var u=users.current();Map<String,Object> row=own(id,u.id());if(!"SUBMITTED".equals(row.get("business_status")))throw new BusinessException(40912,"仅审批中申请可撤回");approvals.withdraw(((Number)row.get("approval_instance_id")).longValue(),u.id());db.update("update leave_application set business_status='CANCELLED',version=version+1,updated_at=current_timestamp where id=?",id);audit.record(u.id(),"LEAVE","WITHDRAW",id,true,null,0);}
